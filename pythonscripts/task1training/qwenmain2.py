...
import os
os.environ["WANDB_PROJECT"]="Qwen2.5NpSfINAL"
max_seq_length = 2048 
dtype = None 
load_in_4bit = True 
...
proof_prompt = """Below is a mathematical statement along with context. Write a detailed and mathematically correct proof that correctly proves the statement.
...
def formatting_prompts_func(examples):
    ...
    statements = examples["text"]    
    ctxs_list  = examples["ctxs"]    
    proofs     = examples["target"]  
...
def init_trainer(model, tokenizer, dataset,eval_dataset):
    ...
    args = TrainingArguments(
        ...
        seed = 3407,
        output_dir = "outputsrun2npsqwen",
        report_to = "wandb",
        run_name="run2"  # Use this for WandB etc
    ),
...
def main():
    ...
    model.save_pretrained("NPSQwenearlystop2") 
    tokenizer.save_pretrained("NPSQwenearlystop2")
