...
def init_trainer(model, tokenizer, dataset,eval_dataset):
    ...
    args = TrainingArguments(
        ...
        seed = 3407,
        output_dir = "outputsrun4llamanaturalproofs",
        report_to = "wandb",
        run_name="run4"  # Use this for WandB etc
    ),
...
def main():
    ...
    trainer = init_trainer(model, tokenizer, dataset,eval_dataset=Dataset.from_pandas(df_valid))

    trainer_stats = trainer.train()
    model.save_pretrained("NLearlystop4") 
    tokenizer.save_pretrained("NLearlystop4")
